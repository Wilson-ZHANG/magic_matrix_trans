...
save_train = open('train2id.txt','a+')
save_test = open('test2id.txt','a+')
save_valid = open('valid2id.txt','a+')
...
def train_embed(input_file1,train_num):
    with open(input_file1,'r',encoding='utf-8') as sf:
        ...
        for ele_tt in tt:
            print(dict[ele_tt.split()[0]],dict[ele_tt.split()[1]],ele_tt.split()[2],file=save_train)
    return train_num
...
def valid_embed(valid_file,valid_num):
    with open(valid_file,'r',encoding='utf-8') as vf:
        ...
        for ele_tt in vvf:
            print(dict[ele_tt.split()[0]],dict[ele_tt.split()[1]],ele_tt.split()[2],file=save_valid)
    return valid_num
...
def test_embed(test_file,test_num):
    with open(test_file,'r',encoding='utf-8') as tf:
        ...
        for ele_tt in tff:
            print(dict[ele_tt.split()[0]],dict[ele_tt.split()[1]],ele_tt.split()[2],file=save_test)
    return test_num
...
#test_num = test_embed(input_file6,test_num)
save_train.close()
save_test.close()
save_valid.close()
